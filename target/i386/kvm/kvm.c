...
{
    return vm_type == KVM_X86_TDX_VM || vm_type == KVM_X86_TDX_VM_OLD;
}
                                     ...
                                     int *max_banks)
{
    ...
    r = kvm_vm_check_extension(s, KVM_CAP_MCE);
    if (r > 0) {
        *max_banks = r;
        r = kvm_vm_ioctl(s, KVM_X86_GET_MCE_CAP_SUPPORTED, mce_cap);
        if (r < 0)
...
int kvm_arch_init(MachineState *ms, KVMState *s)
{
     ...
     * size.
     */
    if (kvm_vm_check_extension(s, KVM_CAP_SET_IDENTITY_MAP_ADDR)) {
