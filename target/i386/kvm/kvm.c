                                           ...
                                           uint32_t len, MemTxAttrs attrs)
{
    ...
    if (ret < 0) {
        uint64_t total_fail_len;

        total_fail_len = len > completed_len ? len - completed_len : len;
        completed_len = len - total_fail_len;
        memset(dest + completed_len, 0, total_fail_len);
    }
