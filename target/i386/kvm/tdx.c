...
bool tdx_cpuid_check_mismatch(X86CPU *cpu, struct kvm_cpuid2 *expected,
                              struct kvm_cpuid2 *actual)
{
    ...
    for (i = 0; i < actual->nent; i++) {
        ...
        if (!expected_e) {
            if (actual_e->eax || actual_e->ebx || actual_e->ecx || actual_e->edx) {
                printf("CPU %d: Unexpected CPUID entry [0x%x 0x%x] with non-zero value enforced by KVM/TDX module:\n"
                       "\teax 0x%08x ebx 0x%08x ecx 0x%08x edx 0x%08x\n",
                       cpu->apic_id, actual_e->function, actual_e->index,
                       actual_e->eax, actual_e->ebx, actual_e->ecx, actual_e->edx);
                /*
                 * For CPUID leaves that QEMU doesn't support yet, just print
                 * the message and allow to continue running.
                 */
                //mismatch = true;
            }
            ...
            if (actual_e->function == 0xd && actual_e->index == 0x0) {
                ...
                /*
                 * TDX module exposes some bit in CPUID.7_2.EDX to guest
