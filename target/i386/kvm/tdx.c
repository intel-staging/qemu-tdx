...
int tdx_pre_create_vcpu(CPUState *cpu, Error **errp)
{
    ...
    if (r < 0) {
        if (!retry) {
            error_append_hint(&local_err, "Hardware RNG (Random Number Generator)"
            " is busy occupied by someone (via RDRAND/RDSEED) maliciously, which "
            "leads to KVM_TDX_INIT_VM keeping failure due to lack of entropy.\n");
        }
        error_propagate(errp, local_err);
        return r;
    }
