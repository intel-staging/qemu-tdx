...
bool kvm_guest_memfd_mmap_supported;
...
static int kvm_init(AccelState *as, MachineState *ms)
{
    ...
    kvm_guest_memfd_mmap_supported =
        kvm_vm_check_extension(s, KVM_CAP_GMEM_MMAP);
...
int kvm_create_guest_memfd(uint64_t size, uint64_t flags, Error **errp)
{
    ...
    if (flags & GUEST_MEMFD_FLAG_MMAP && !kvm_guest_memfd_mmap_supported) {
        error_setg(errp, "KVM does not support guest_memfd with mmap");
        return -1;
    }
